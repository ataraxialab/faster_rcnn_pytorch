...
class RPN(nn.Module):
    ...
    def forward(self, im_data, im_info, gt_boxes=None, gt_ishard=None, dontcare_areas=None):
        print 'im_data device', im_data.get_device()
        print 'im_info device', im_info.get_device()
        ...
        if self.training:
            ...
            rpn_data = self.anchor_target_layer(rpn_cls_score, gt_boxes, gt_ishard, dontcare_areas,
                                                im_info.cpu().data.numpy(), self._feat_stride, self.anchor_scales)
            self.cross_entropy, self.loss_box = self.build_loss(rpn_cls_score_reshape, rpn_bbox_pred, rpn_data, im_data.get_device())

        return features, rois

    def build_loss(self, rpn_cls_score_reshape, rpn_bbox_pred, rpn_data, device_id=0):
        # classification loss
        rpn_cls_score = rpn_cls_score_reshape.permute(0, 2, 3, 1).contiguous().view(-1, 2)
        rpn_label = rpn_data[0].view(-1).cuda(device_id)

        rpn_keep = Variable(rpn_label.data.ne(-1).nonzero().squeeze()).cuda(device_id)
        print 'rpn keep device:', rpn_keep.get_device(), 'rpn_cls_score', rpn_cls_score.get_device()
        rpn_cls_score = torch.index_select(rpn_cls_score, 0, rpn_keep)
        print 'rpn_label', rpn_label.get_device()
        ...
        rpn_bbox_inside_weights = rpn_bbox_inside_weights.cuda(device_id)
        rpn_bbox_outside_weights = rpn_bbox_outside_weights.cuda(device_id)
        rpn_bbox_targets = rpn_bbox_targets.cuda(device_id)
...
#        self.add_module('rpn', self.rpn)
#        self.add_module('resnet152', self.res5_features)
#        self.add_module('score_fc', self.score_fc)
#        self.add_module('bbox_fc', self.bbox_fc)
    ...
    def forward(self, im_data, im_info, gt_boxes=None, gt_ishard=None, dontcare_areas=None):
        dontcare_areas = None if dontcare_areas is None else dontcare_areas.cpu().data.numpy()
        gt_ishard = None if gt_ishard is None else gt_ishard.cpu().data.numpy()
        gt_boxes= None if gt_boxes is None else gt_boxes.cpu().data.numpy()
        ...
        res5_features = self.res5_features(pooled_features.cuda(im_data.get_device()),'layer4')
        ...
        if self.training:
            self.cross_entropy, self.loss_box = self.build_loss(cls_score, bbox_pred, roi_data, im_data.get_device())

        return cls_prob, bbox_pred, rois

    def build_loss(self, cls_score, bbox_pred, roi_data, device_id=0):
        # classification loss
        label = roi_data[1].squeeze().cuda(device_id)
        ...
        ce_weights = ce_weights.cuda(device_id)
